...
const tierRepo = () => AppDataSource.getRepository(ObsHumedalTier);
const contribRepo = () => AppDataSource.getRepository(ObsHumedalContributor);
...
export async function createTier(data: Partial<ObsHumedalTier>) {
...
export async function updateTier(id: number, data: Partial<ObsHumedalTier>) {
...
export async function deleteTier(id: number) {
  const t = await getTier(id);
  // No borramos en duro: marcamos archivado.
  t.archived = true;
  t.visible = false;
  return tierRepo().save(t);
}
...
export async function createContributor(data: Partial<ObsHumedalContributor>) {
...
export async function updateContributor(id: number, data: Partial<ObsHumedalContributor>) {
...
// Cuando se aprueba un prospecto con contributorId, sumamos al contador de
// validatedContributions del contribuyente y recalculamos su score y tier.
export async function incrementContributorOnApproval(contributorId: number) {
  ...
  // Score = validados * 10 (peso simple, ajustable). Tier se recalcula.
