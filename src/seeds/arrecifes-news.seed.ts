...
// Upsert por slug (artículos) y urlHash (prospectos).
//
// Trade-off: re-correr `npm run seed` SOBRESCRIBE las ediciones que el admin
// haya hecho a estos 3 artículos canónicos desde /admin/news. Los artículos
// que el admin haya CREADO desde la UI (con slug distinto) no se tocan.
// Si necesitas customizar uno de los canónicos para producción, cámbiale el
// slug — quedará protegido del re-seed.
...
const CANONICAL_PROSPECTS = [
  {
    ...
    url: 'https://es.mongabay.com/2026/04/mexico-comunidades-fallo-descontaminar-rios/',
...
export async function seedArrecifesNews() {
  ...
  for (const a of CANONICAL_ARTICLES) {
    const existing = await newsRepo.findOne({ where: { slug: a.slug } });
    if (existing) {
      Object.assign(existing, a);
      await newsRepo.save(existing);
      updated++;
    } else {
      await newsRepo.save(newsRepo.create(a));
      created++;
    }
  }
  console.log(`  ObsReefNews: ${created} creado(s), ${updated} actualizado(s)`);

  let pCreated = 0;
  let pSkipped = 0;
  for (const p of CANONICAL_PROSPECTS) {
    const urlHash = crypto.createHash('sha256').update(p.url).digest('hex');
    const existing = await prospectRepo.findOne({ where: { urlHash } });
    if (existing) {
      pSkipped++;
      continue;
    }
  ...
  console.log(`  ObsReefNewsProspect: ${pCreated} creado(s), ${pSkipped} ya existían`);
}
