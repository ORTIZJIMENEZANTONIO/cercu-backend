...
- `1742000000000-SeedTechosVerdesExtendedCms.ts` — siembra las secciones del CMS añadidas en la reorganización IA del observatorio de techos verdes (Fases 1-4 + sweep editorial):
  - **home**: `audienceGate` (4 puertas Aprende/Explora/Investigación/Comunidad), `academicHighlight` (banner del paper Q1 Cervantes-Nájera 2025 SCS con DOI + 3 KPIs).
  - **aprende** (hub educativo): `hero`, `intro`, `capas` (6 capas del techo verde), `tipologias` (extensivo/semi-intensivo/intensivo con peso saturado/sustrato/vegetación/mantenimiento), `beneficios` (4: térmico/hídrico/atmosférico/social), `casoIntro` + `casoCiiemad` (6 datos del experimento), `galeria` (10 fotos del techo CIIEMAD: PPTX 2023 + INFORME PARCIAL + vista aérea de la tesis), `cta`.
  - **investigacion** (hub académico): `hero`, `pesosAhp` (8 variables AHP con peso %).
  - **agenda-2030**: `ods` (7 ODS conectados con num, color oficial ONU, título, vía, rol MEA, cita académica + array de metas), `datos` (6 stats académicos con target/display + unidad + fuente + href).
  - **indicadores**: `kpis` (8 fun-cards del header con label, valor, unidad, icono fun-palette, color, cambio opcional).
  - **metodologia.pasos**: shape extendido con `number + title + description + icono + color + details[]`. La migración usa el helper `upsertIfStaleShape()` que detecta si el primer item tiene el campo `number` (= shape nuevo) — si no, REEMPLAZA porque el seed 1740 tenía shape simple sin `number/icono/color`. Patrón único en el repo: las demás secciones usan `insertIfMissing()` (preserva ediciones).
  - Marca `updatedBy='migration:1742'`. `down()` solo elimina filas con ese `updatedBy` — preserva ediciones del editor admin.
  - Co-existe con la migración del mismo timestamp `AddSourceIndexToProspects` (timestamps gemelos son OK en TypeORM mientras los `name` difieran).
...
**Lista oficial `ADMIN_PERMISSIONS` (`src/modules/observatory/admin/observatory-admin.validation.ts:163-173`):**
- Universales: `manage_users`, `manage_cms`
- Humedales: `manage_humedales`, `manage_hallazgos`, `manage_notihumedal`, `manage_prospectos`
- Techos verdes: `manage_techos`, `manage_candidatos`, `manage_validaciones`
- Arrecifes: `manage_reefs`, `manage_layers`, `manage_observations`, `manage_conflicts`, `manage_contributors`, `review_submissions`

Si el frontend manda un permiso fuera de esta lista, el Joi schema rechaza con `permissions[N] must be one of [...]` y NO se puede crear/actualizar el usuario. Cualquier UI nueva que invente un permiso (`manage_candidatos`/`manage_validaciones` en techos-verdes UI) DEBE añadirse aquí primero.

**Permisos del CMS (endpoint admin):** `PUT /:observatory/admin/cms/:pageSlug/:sectionKey` solo requiere `auth` middleware — NO chequea `manage_cms` específicamente. Cualquier admin autenticado puede guardar contenido CMS. El `hasPermission()` del frontend (`stores/auth.ts:22`) retorna `true` para superadmin automáticamente; los gates en el editor (`/admin/contenido/[pageSlug].vue`) están limitados a estado de carga (`saving === sectionKey`).
